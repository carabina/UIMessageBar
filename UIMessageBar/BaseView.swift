...
//
//  UIMessageBaseView.swift
//  UIMessageBar
//
//  Created by Elias Abel on 8/17/16.
...
/*
 The `UIMessageBaseView` class is a reusable message view base class that implements some
 of the optional UIMessageBar protocols and provides some convenience functions
 and a configurable tap handler. Message views do not need to inherit from `BaseVew`.
 */
open class UIMessageBaseView: UIView, BackgroundViewable, MarginAdjustable {
     ...
     */
    open var tapHandler: ((_ view: UIMessageBaseView) -> Void)? {
        didSet {
            installTapRecognizer()
        }
    }
    ...
    fileprivate lazy var tapRecognizer: UITapGestureRecognizer = {
        let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(UIMessageView.tapped))
    ...
    /**
     ...
     value is added to adjustments already made by UIMessageBar in case the
    ...
    /**
     ...
     by UIMessageBar in case the defaults don't work for a particular layout.
     */
