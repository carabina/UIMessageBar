...
private let globalMessageBarInstance = UIMessageBar()

/**
 The `UIMessageBar` class provides the interface for showing and hiding messages.
 It behaves like a queue, only showing one message at a time. Message views that
 adopt the `Identifiable` protocol (as `UIMessageView` does) will have duplicates removed.
 */
open class UIMessageBar {
    
    public typealias BarActionClosure = ((_ view: UIMessageView, _ button: UIButton) -> Swift.Void)
    ...
    public enum PresentationContext {
        ...
        */
        ...
        /**
         ...
         to display over. When displaying under the status bar, UIMessageBar automatically
         increases the top margins of any message view that adopts the `MarginInsetting`
         protocol (as `UIMessageView` does) to account for the status bar.
        */
    ...
    public enum DimMode {
        ...
        */
        ...
        /**
         ...
         UIMessageBar does not apply alpha transparency to the color, so any alpha
    ...
    */
     ...
     */
    public typealias EventListener = (UIMessageBar.Event) -> Swift.Void
    
    /**
     The `UIMessageBarConfig` struct specifies options for displaying a single message view. It is
     provided as an optional argument to one of the `UIMessageView.show()` methods.
     */
    public struct UIMessageBarConfig {
        ...
        /**
         ...
         protocol (as `UIMessageView` does), the pan gesture recognizer is installed
         ...
         the message view itself. The default is `false`.
        */
        public var interactiveHide = false
     ...
     */
    open func show(config: UIMessageBarConfig, view: UIView) {
        let presenter = UIMessagePresenter(config: config, view: view, delegate: self)
        messageQueue.sync {
     ...
     */
    open func show(config: UIMessageBarConfig, viewProvider: @escaping ViewProvider) {
    ...
    /**
     ...
     views, such as `UIMessageView`, that adopt the `Identifiable` protocol.
     ...
     */
    public var defaultConfig = UIMessageBarConfig()
    ...
    fileprivate class Delays {
        ...
        fileprivate func add(presenter: UIMessagePresenter) {
            ids.insert(presenter.id)
        }

        @discardableResult
        fileprivate func remove(presenter: UIMessagePresenter) -> Bool {
    ...
    fileprivate let messageQueue = DispatchQueue(label: "it.swiftkick.UIMessageBar", attributes: [])
    fileprivate var queue: [UIMessagePresenter] = []
    fileprivate var delays = Delays()
    fileprivate var counts: [String : Int] = [:]
    fileprivate var _current: UIMessagePresenter? = nil {
        didSet {
            if oldValue != nil {
    ...
    fileprivate func enqueue(presenter: UIMessagePresenter) {
...
extension UIMessageBar: PresenterDelegate {

    func hide(presenter: UIMessagePresenter) {
        messageQueue.sync {
    ...
    private func presenter(forAnimator animator: Animator) -> UIMessagePresenter? {
...
/**
 ...
 UIMessageBar provides several default nib files in the Resources folder that can be
...
extension UIMessageBar {
    ...
    /**
     ...
     first followed by the UIMessageBar bundle.
    ...
    /**
     ...
     with matching type `T`. The main bundle is searched first followed by the UIMessageBar bundle.
    ...
    fileprivate class func internalViewFromNib<T: UIView>(named name: String, bundle: Bundle? = nil, filesOwner: AnyObject = NSNull.init()) throws -> T {
        ...
        if let bundle = bundle {
            ...
            if Bundle.main.path(forResource: name, ofType: "nib") != nil {
                ...
                resolvedBundle = Bundle.frameworkBundle()
            }
        }
        let arrayOfViews = resolvedBundle.loadNibNamed(name, owner: filesOwner, options: nil) ?? []
        guard let view = arrayOfViews.flatMap( { $0 as? T} ).first else { throw UIMessageBarError.cannotLoadViewFromNib(nibName: name) }
        return view
    }
}
...
/*
 ...
 This extension provides a shared instance of `UIMessageBar` and a static API wrapper around
 this instance for simplified syntax. For example, `UIMessageBar.show()` is equivalent
 to `UIMessageBar.sharedInstance.show()`.
 */

extension UIMessageBar {
    
    /**
     A default shared instance of `UIMessageBar`. The `UIMessageBar` class provides
     a set of static APIs that wrap calls to this instance. For example, `UIMessageBar.show()`
     is equivalent to `UIMessageBar.sharedInstance.show()`.
     */
    public static var sharedInstance: UIMessageBar {
        return globalMessageBarInstance
    }
    
    public static func show(viewProvider: @escaping ViewProvider) {
        globalMessageBarInstance.show(viewProvider: viewProvider)
    }
    
    public static func show(config: UIMessageBarConfig, viewProvider: @escaping ViewProvider) {
        globalMessageBarInstance.show(config: config, viewProvider: viewProvider)
    }
    
    public static func show(view: UIView) {
        globalMessageBarInstance.show(view: view)
    }

    public static func show(config: UIMessageBarConfig, view: UIView) {
        globalMessageBarInstance.show(config: config, view: view)
    }

    public static func hide() {
        globalMessageBarInstance.hide()
    }
    
    public static func hideAll() {
        globalMessageBarInstance.hideAll()
    }
    
    public static func hide(id: String) {
        globalMessageBarInstance.hide(id: id)
    }

    public static func hideCounted(id: String) {
        globalMessageBarInstance.hideCounted(id: id)
    }

    public static var defaultConfig: UIMessageBarConfig {
        get {
            return globalMessageBarInstance.defaultConfig
        }
        set {
            globalMessageBarInstance.defaultConfig = newValue
        }
    }
    
    public static var pauseBetweenMessages: TimeInterval {
        get {
            return globalMessageBarInstance.pauseBetweenMessages
        }
        set {
            globalMessageBarInstance.pauseBetweenMessages = newValue
        }
    }

    public static func current<T: UIView>(id: String) -> T? {
        return globalMessageBarInstance.current(id: id)
    }

    public static func queued<T: UIView>(id: String) -> T? {
        return globalMessageBarInstance.queued(id: id)
    }

    public static func currentOrQueued<T: UIView>(id: String) -> T? {
        return globalMessageBarInstance.currentOrQueued(id: id)
    }

    public static func count(id: String) -> Int {
        return globalMessageBarInstance.count(id: id)
    }

    public static func set(count: Int, for id: String) {
        globalMessageBarInstance.set(count: count, for: id)
    }
}
