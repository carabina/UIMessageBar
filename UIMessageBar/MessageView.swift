...
//  UIMessageBar
//
//  Created by Elias Abel on 7/30/16.
...
/*
 */
open class UIMessageView: UIMessageBaseView, Identifiable, AccessibleMessage {
    ...
    open var buttonTapHandler: UIMessageBar.BarActionClosure?
    
    @objc func buttonTapped(_ button: UIButton) {
        buttonTapHandler?(self, button)
    }
    ...
    @IBOutlet open var button: UIButton? {
        didSet {
            if let old = oldValue {
                old.removeTarget(self, action: #selector(UIMessageView.buttonTapped(_:)), for: .touchUpInside)
            }
            if let button = button {
                button.addTarget(self, action: #selector(UIMessageView.buttonTapped(_:)), for: .touchUpInside)
            }
        }
    }
    ...
    open var id: String {
        get {
            return customId ?? "UIMessageView:title=\(String(describing: titleLabel?.text)), body=\(String(describing: bodyLabel?.text))"
        }
        set {
            customId = newValue
        }
...
/*
 ...
 `UIMessageView` from the included nib files in a type-safe way. These nib 
 ...
 copied the nib files because UIMessageBar looks for them in the main
 bundle first. See `UIMessageBar` for additional nib loading options.
 */
...
extension UIMessageView {
    ...
    public enum Layout: String {
        ...
        /**
         ...
         container view.
         */
        case messageView = "UIMessageView"
        ...
        /**
         A standard message view like `UIMessageView`, but without
    ...
    /**
     ...
     view found in the nib file with the matching type `T: UIMessageView`.
     
     - Parameter layout: The `Layout` option to use.
     - Parameter filesOwner: An optional files owner.
     
     - Returns: An instance of generic view type `T: UIMessageView`.
     */
    public static func viewFromNib<T: UIMessageView>(layout: Layout, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! UIMessageBar.viewFromNib(named: layout.rawValue)
    }
    
    /**
     Loads the nib file associated with the given `Layout` from
     the given bundle and returns the first view found in the nib
     file with the matching type `T: UIMessageView`.
     ...
     - Returns: An instance of generic view type `T: UIMessageView`.
     */
    public static func viewFromNib<T: UIMessageView>(layout: Layout, bundle: Bundle, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! UIMessageBar.viewFromNib(named: layout.rawValue, bundle: bundle, filesOwner: filesOwner)
    }
}
...
extension UIMessageView {
     ...
     */
    public func configureTheme(_ theme: UIMessageBar.Theme, iconStyle: UIMessageBar.IconStyle = .default) {
...
/*
 ...
 UIMessageBar does not try to be clever by adjusting the layout based on 
...
extension UIMessageView {
     ...
     */
    public func configureContent(title: String?, body: String?, iconImage: UIImage?, iconText: String?, buttonImage: UIImage?, buttonTitle: String?, action: UIMessageBar.BarActionClosure?) {
        titleLabel?.text = title
        bodyLabel?.text = body
        iconImageView?.image = iconImage
        iconLabel?.text = iconText
...
extension UIMessageView {
    ...
    /**
     ...
     have `backgroundView` as a subview of `UIMessageView`, the background view should
