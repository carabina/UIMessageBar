...
class UIMessagePresenter: NSObject {
    ...
    lazy var maskingView: UIMessageMaskingView = { return UIMessageMaskingView() }()
    var presentationContext = PresentationContext.viewController(Weak<UIViewController>(value: nil))
    let animator: Animator

    init(config: UIMessageBar.UIMessageBarConfig, view: UIView, delegate: PresenterDelegate) {
        self.config = config
        self.view = view
        self.delegate = delegate
        self.animator = UIMessagePresenter.animator(forPresentationStyle: config.presentationStyle, delegate: delegate)
    ...
    private static func animator(forPresentationStyle style: UIMessageBar.PresentationStyle, delegate: AnimationDelegate) -> Animator {
        switch style {
        case .top:
            return UIMessageTopBottomAnimation(style: .top, delegate: delegate)
        case .bottom:
            return UIMessageTopBottomAnimation(style: .bottom, delegate: delegate)
        case .center:
            return UIMessagePhysicsAnimation(delegate: delegate)
    ...
    func hide(completion: @escaping AnimationCompletion) {
        ...
        animator.hide(context: context) { (completed) in
            if let viewController = self.presentationContext.viewControllerValue() as? UIMessageWindowViewController {
    ...
    private func safeZoneConflicts() -> SafeZoneConflicts {
        ...
        let inNormalWindowLevel: Bool = {
            if let vc = presentationContext.viewControllerValue() as? UIMessageWindowViewController {
        ...
        let underNavigationBar: Bool = {
            if let vc = presentationContext.viewControllerValue() as? UINavigationController { return vc.ifVisible(view: vc.navigationBar) }
            return false
        }()
        let underTabBar: Bool = {
            if let vc = presentationContext.viewControllerValue() as? UITabBarController { return vc.ifVisible(view: vc.tabBar) }
            return false
        }()
        if #available(iOS 11, *) {
    ...
    private func getPresentationContext() throws -> PresentationContext {
        ...
        func newWindowViewController(_ windowLevel: UIWindowLevel) -> UIViewController {
            let viewController = UIMessageWindowViewController(windowLevel: windowLevel, config: config)
        ...
        case .automatic:
            if let rootViewController = UIApplication.shared.keyWindow?.rootViewController {
                let viewController = rootViewController.selectPresentationContextTopDown(config)
                return .viewController(Weak(value: viewController))
            } else {
                throw UIMessageBarError.noRootViewController
            }
        case .window(let level):
            let viewController = newWindowViewController(level)
            return .viewController(Weak(value: viewController))
        case .viewController(let viewController):
            let viewController = viewController.selectPresentationContextBottomUp(config)
            return .viewController(Weak(value: viewController))
        case .view(let view):
    ...
    func install() {
        ...
        func topLayoutConstraint(view: UIView, containerView: UIView, viewController: UIViewController?) -> NSLayoutConstraint {
            if case .top = config.presentationStyle, let nav = viewController as? UINavigationController, nav.ifVisible(view: nav.navigationBar) {
        ...
        func bottomLayoutConstraint(view: UIView, containerView: UIView, viewController: UIViewController?) -> NSLayoutConstraint {
            if case .bottom = config.presentationStyle, let tab = viewController as? UITabBarController, tab.ifVisible(view: tab.tabBar) {
        ...
        if let windowViewController = presentationContext.viewControllerValue() as? UIMessageWindowViewController {
